...
export const insuranceCarriers = pgTable(
  ...
  (table) => {
    return {
      idxCarriersCode: index('idx_carriers_code').using(
        'btree',
        table.carrierCode.asc().nullsLast(),
      ),
      idxCarriersStates: index('idx_carriers_states').using(
        'gin',
        table.supportedStates.asc().nullsLast(),
      ),
      insuranceCarriersCarrierCodeKey: unique(
        'insurance_carriers_carrier_code_key',
      ).on(table.carrierCode),
    };
  },
);
...
export const agencies = pgTable(
  ...
  (table) => {
    return {
      idxAgenciesCode: index('idx_agencies_code').using(
        'btree',
        table.agencyCode.asc().nullsLast(),
      ),
      idxAgenciesSubscription: index('idx_agencies_subscription').using(
        'btree',
        table.subscriptionStatus.asc().nullsLast(),
      ),
      agenciesAgencyCodeKey: unique('agencies_agency_code_key').on(
        table.agencyCode,
      ),
    };
  },
...
export const profiles = pgTable(
  'profiles',
  {
    ...
    email: text().notNull(),
    userType: text('user_type').default('regular'),
  ...
  (table) => {
    return {
      ...
      idxProfilesAgency: index('idx_profiles_agency').using(
        'btree',
        table.agencyId.asc().nullsLast(),
      ),
      profilesAgencyIdAgenciesIdFk: foreignKey({
...
export const insuranceContacts = pgTable(
  'insurance_contacts',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    agencyId: uuid('agency_id'),
  ...
  (table) => {
    return {
      idxContactsActive: index('idx_contacts_active').using(
        'btree',
        table.isActive.asc().nullsLast(),
      ),
      idxContactsAgency: index('idx_contacts_agency').using(
        'btree',
        table.agencyId.asc().nullsLast(),
      ),
      idxContactsEmail: index('idx_contacts_email').using(
        'btree',
        table.email.asc().nullsLast(),
      ),
      idxContactsType: index('idx_contacts_type').using(
        'btree',
        table.contactType.asc().nullsLast(),
      ),
...
export const policies = pgTable(
  ...
  (table) => {
    return {
      idxPoliciesAgency: index('idx_policies_agency').using(
        'btree',
        table.agencyId.asc().nullsLast(),
      ),
      idxPoliciesCarrier: index('idx_policies_carrier').using(
        'btree',
        table.carrier.asc().nullsLast(),
      ),
      idxPoliciesCoveragesGin: index('idx_policies_coverages_gin').using(
        'gin',
        table.coverages.asc().nullsLast(),
      ),
      idxPoliciesDataSource: index('idx_policies_data_source').using(
        'btree',
        table.dataSource.asc().nullsLast(),
      ),
      idxPoliciesEffectiveDate: index('idx_policies_effective_date').using(
        'btree',
        sql`(policy_period ->> 'effective_date'::text)`,
      ),
      idxPoliciesEndorsementsGin: index('idx_policies_endorsements_gin').using(
        'gin',
        table.endorsements.asc().nullsLast(),
      ),
      idxPoliciesPolicyNumber: index('idx_policies_policy_number').using(
        'btree',
        table.policyNumber.asc().nullsLast(),
      ),
      idxPoliciesPolicyType: index('idx_policies_policy_type').using(
        'btree',
        table.policyType.asc().nullsLast(),
      ),
      idxPoliciesPolicyholderId: index('idx_policies_policyholder_id').using(
        'btree',
        table.policyholderId.asc().nullsLast(),
      ),
      idxPoliciesPremiumGin: index('idx_policies_premium_gin').using(
        'gin',
        table.premium.asc().nullsLast(),
      ),
      idxPoliciesPropertyGin: index('idx_policies_property_gin').using(
        'gin',
        table.property.asc().nullsLast(),
      ),
      idxPoliciesState: index('idx_policies_state').using(
        'btree',
        sql`((property -> 'address'::text) ->> 'state'::text)`,
      ),
      idxPoliciesStatus: index('idx_policies_status').using(
        'btree',
      ...
      policiesPolicyNumberKey: unique('policies_policy_number_key').on(
        table.policyNumber,
      ),
    };
  },
...
export const policyholders = pgTable(
  'policyholders',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => {
    return {
      idxPolicyholdersEmail: index('idx_policyholders_email').using(
        'btree',
        table.email.asc().nullsLast(),
      ),
      idxPolicyholdersHolderId: index('idx_policyholders_holder_id').using(
        'btree',
        table.holderId.asc().nullsLast(),
      ),
      idxPolicyholdersLastName: index('idx_policyholders_last_name').using(
        'btree',
